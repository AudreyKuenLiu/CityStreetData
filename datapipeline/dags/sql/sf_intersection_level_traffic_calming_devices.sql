...
SELECT
    CASE WHEN 
        (dta.install_date = '01/01/1900 12:00:00 AM' OR dta.install_date is NULL) AND 
        validate_sf_intersection_level_traffic_calming_devices_fiscal_year(fiscal_yr) THEN
            datetime(format_sf_intersection_level_traffic_calming_devices_fiscal_year(fiscal_yr) || '-01-01 00:00:00')
...
WHERE
    sf.cnn = dta.cnn
    AND
    CAST(sf.metadata->>'intersection_row_id' AS INTEGER) = dta.row_id
    AND
    sf.metadata != json_object(
        'num_units', dta.units,
        'fiscal_yr', dta.fiscal_yr,
        'install_date', dta.install_date,
        'program', dta.program,
