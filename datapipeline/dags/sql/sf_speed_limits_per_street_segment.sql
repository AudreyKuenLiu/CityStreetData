...
WITH data_to_add AS (
    ...
    FROM (
        SELECT
            ...
            mtab_reso_text, 
            install_date,
            MAX(COALESCE(NULLIF(install_date,0), NULLIF(mtab_date, 0), 0)) OVER(PARTITION BY cnn) as assumed_install_date, 
            status, 
            workorder, 
            MLineFromText(shape, 4326) as shape, 
            data_as_of, 
            data_loaded_at, 
...
FROM
    data_to_add as dta
    LEFT JOIN
    most_recent_speedlimit_change as slc ON slc.cnn = dta.cnn AND ST_Equals(MLineFromText(slc.metadata->>'$.shape', 4326), dta.shape)
WHERE
    slc.cnn IS NULL
    OR
...
FROM
    data_to_add as dta
    LEFT JOIN
    most_recent_schoolzone_change as szc ON szc.cnn = dta.cnn AND ST_Equals(MLineFromText(szc.metadata->>'$.shape', 4326), dta.shape)
WHERE
    szc.cnn IS NULL
    OR
