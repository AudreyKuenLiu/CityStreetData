...
SELECT
  ...
  supervisor_district, 
  ST_LineFromText(line, 4326) as line
FROM
...
SET 
  ...
  line = ST_LineFromText(dtu.line, 4326)
FROM
...
WHERE
  ...
  AND (
    ...
    OR
    NOT ST_Equals(sfsi.line, ST_LineFromText(dtu.line, 4326))
  );
...
most_recent_classcode AS (
  ...
  *
  FROM ( 
    ...
    FROM
      sf_street_feature_classcode
    )
  WHERE 
    rn = 1
)
INSERT INTO sf_street_feature_classcode(
  cnn,
  completed_at,
  value,
  is_active
...
SELECT
  ...
  CASE WHEN mrc.cnn is NULL THEN COALESCE(dta.date_added, dta.data_as_of)
       ...
       ELSE dta.data_as_of
  END as completed_at,
  integer_to_class_code(CAST(dta.classcode AS INTEGER)) as value,
  dta.active as metadata
FROM
  data_to_add dta
  LEFT JOIN
...
WHERE
  mrc.cnn is NULL
  OR
  integer_to_class_code(CAST(dta.classcode AS INTEGER)) != mrc.value
;
